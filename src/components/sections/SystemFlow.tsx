      ...
      <div className="container mx-auto px-6 md:px-12">
        <div className="mb-14 flex flex-col gap-6 md:mb-20 md:flex-row md:items-end md:justify-between">
          <div className="max-w-2xl">
            ...
            <h2 className="text-4xl md:text-6xl font-black leading-none tracking-[-0.04em] text-primary uppercase">
          ...
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-7 lg:gap-3">
            {flowSteps.map((step, idx) => (
              <motion.div
                ...
                className="relative"
              >
                <div className="relative z-10 flex min-h-[84px] items-center justify-between rounded-[20px] border border-border/60 bg-white px-4 py-4 shadow-[0_10px_30px_rgba(0,0,0,0.04)] lg:min-h-[112px] lg:flex-col lg:items-start lg:justify-between">
                  <span className="font-mono text-[10px] font-bold uppercase tracking-[0.24em] text-muted">
                    {String(idx + 1).padStart(2, '0')}
                  </span>
                  <span className="max-w-[12rem] text-right font-mono text-xs font-bold uppercase tracking-wider text-primary lg:text-left leading-snug">
                    {step}
                  </span>
                </div>
                {idx < flowSteps.length - 1 && (
                  <div className="flex justify-center py-2 text-primary/35 lg:absolute lg:right-[-1.15rem] lg:top-1/2 lg:z-20 lg:-translate-y-1/2 lg:bg-surface lg:px-1.5 lg:py-0">
                    <ArrowRight size={15} />
                  </div>
                )}
              </motion.div>

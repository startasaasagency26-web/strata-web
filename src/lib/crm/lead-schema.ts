...
export const normalizePhone = (value: unknown) => {
  ...
  if (digits.length >= 8 && digits.length <= 11) return `+60${digits}`;
...
export const validateLeadPayload = (
  ...
  if (!roleInBusiness) fieldErrors.roleInBusiness = "Role in business is required.";
  if (!countryTimezone) fieldErrors.countryTimezone = "Country / timezone is required.";
  if (!preferredLanguage) fieldErrors.preferredLanguage = "Preferred language is required.";

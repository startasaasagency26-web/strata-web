    ...
    <CrmShell>
      <div className="space-y-12">
        <div>
          ...
          <p className="text-[10px] font-mono font-bold tracking-[0.3em] text-muted uppercase">Internal configuration & backend status</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-10">
            <section className="space-y-6">
              ...
              <div className={cn(
                ...
                settings.isConfigured
                  ? 'bg-positiveSoft/80 backdrop-blur-sm border-positive/30/50'
                  : 'bg-cautionSoft/80 backdrop-blur-sm border-caution/30/50'
              )}>
                <div className="flex items-center gap-6 text-center md:text-left">
                  <div className={cn(
                    'w-16 h-16 rounded-full flex items-center justify-center shadow-sm bg-surface border border-border',
                    settings.isConfigured ? 'text-positive' : 'text-caution'
                  )}>
            ...
            <section className="space-y-6">
              ...
              <div className="bg-surface2 backdrop-blur-md border border-border rounded-[32px] overflow-hidden shadow-sm">
                <div className="divide-y divide-white/40">
                  {team.length > 0 ? (
                    team.map((member) => (
                      <div key={member.id} className="px-8 py-6 flex items-center justify-between group">
                        <div className="flex items-center gap-5">
                          <div className="w-12 h-12 rounded-[16px] bg-surface border border-border shadow-sm flex items-center justify-center font-display text-sm font-bold uppercase text-text">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              {member.id === profile?.id && (
                                <span className="text-[8px] font-mono bg-surface2 text-muted px-1.5 py-0.5 rounded uppercase font-bold tracking-widest">You</span>
                              )}
                            </div>
                            <p className="text-[10px] font-mono text-muted uppercase tracking-widest font-bold mt-0.5">{member.role} · {member.status}</p>
                          </div>
                        </div>
                        {permissions.canChangeRoles && member.id !== profile?.id && (
                          <button
                            onClick={() => openManageMember(member)}
                            className="text-[10px] font-mono font-bold tracking-widest text-muted hover:text-text uppercase transition-colors px-4 py-2 rounded-xl hover:bg-surface3 border border-transparent hover:border-border hover:shadow-sm"
                          >
                            Manage
                          </button>
                        )}
                    ...
                    <div className="px-8 py-12 text-center text-muted font-mono text-[10px] uppercase tracking-widest font-bold">
                  ...
                  {permissions.canManageTeam && (
                    <div className="px-8 py-6 text-center">
                      <button
                        ...
                        className="w-full py-4 text-[10px] font-mono font-bold tracking-widest text-muted/60 uppercase transition-all flex items-center justify-center gap-2 cursor-not-allowed"
                      >
                        <Plus size={14} /> Invite Team Member
                      </button>
                      <p className="mt-2 text-[9px] font-mono font-bold uppercase tracking-widest text-muted">Invite backend not connected yet.</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-10">
            <section className="space-y-6">
              ...
              <div className="bg-surface2 backdrop-blur-md border border-border rounded-[32px] p-8 space-y-6 shadow-sm">
                ...
                {!permissions.canEditSettings && (
                  <p className="text-[10px] font-mono font-bold text-muted uppercase tracking-widest text-center">Admin access required to update endpoints.</p>
                )}
            ...
            {permissions.canAccessDangerZone && (
              <section className="rounded-[32px] p-8 space-y-4 transition-all shadow-sm bg-dangerSoft/80 backdrop-blur-sm border border-danger/30/50">
                <div className="flex items-center gap-2 text-danger">
                  <AlertTriangle size={18} />
                  <h3 className="text-xs font-mono font-bold uppercase tracking-widest">Danger Zone</h3>
                </div>
                <p className="text-[10px] text-danger/80 leading-relaxed uppercase font-mono font-bold tracking-widest">Irreversible system actions</p>
                <button
                  onClick={() => setShowResetModal(true)}
                  className="w-full py-4 rounded-xl border border-danger/30/20 bg-surface shadow-sm text-danger text-[10px] font-mono font-bold uppercase tracking-widest hover:bg-dangerSoft hover:border-danger/30 transition-all"
                >
                  Factory Reset CRM
                </button>
              </section>
      ...
      {showIntegrationModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gold/20 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="integration-status-title">
          <div ref={integrationRef} className="w-full max-w-lg rounded-[24px] border border-border bg-surface2 p-6 shadow-xl backdrop-blur-md space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowIntegrationModal(false)} aria-label="Close integration status" className="w-9 h-9 rounded-full hover:bg-surface2 flex items-center justify-center">
                <X size={16} />
              </button>
            </div>
            {isCheckingHealth ? (
              <div className="flex items-center gap-3 rounded-xl bg-surface2 p-4 text-sm font-bold text-text2">
                <Loader2 size={16} className="animate-spin" /> Checking protected backend status...
              </div>
            ) : healthError ? (
              <div className="rounded-xl border border-danger/30 bg-dangerSoft p-4 text-sm font-bold text-danger">{healthError}</div>
            ) : healthStatus ? (
              <div className="space-y-3">
                <div className={cn('rounded-xl border p-4 text-sm font-bold', healthStatus.supabaseConnected ? 'border-positive/30 bg-positiveSoft text-positive' : 'border-danger/30 bg-dangerSoft text-danger')}>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {Object.entries(healthStatus.tables).map(([table, ok]) => (
                    <div key={table} className="flex items-center justify-between rounded-xl border border-border bg-surface2 px-4 py-3 text-xs font-bold text-text2">
                      <span>{table}</span>
                      <span className={ok ? 'text-positive' : 'text-danger'}>{ok ? 'OK' : 'Unavailable'}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {selectedMember && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gold/20 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="manage-member-title">
          <form ref={manageRef} onSubmit={handleSaveMember} className="w-full max-w-md rounded-[24px] border border-border bg-surface2 p-6 shadow-xl backdrop-blur-md space-y-5">
            <div className="flex items-center justify-between">
              <h2 id="manage-member-title" className="text-xl font-display font-bold uppercase tracking-tight text-text">Manage Member</h2>
              <button type="button" onClick={() => setSelectedMember(null)} aria-label="Close manage member modal" className="w-9 h-9 rounded-full hover:bg-surface2 flex items-center justify-center">
                <X size={16} />
              </button>
            </div>
            <p className="text-sm font-bold text-text2">{selectedMember.fullName}</p>
            ...
            <div className="flex justify-end gap-3">
              <button type="button" onClick={() => setSelectedMember(null)} disabled={isSavingMember} className="rounded-full border border-border px-5 py-2.5 text-[10px] font-mono font-bold uppercase tracking-widest text-text hover:bg-surface2">
                Cancel
              </button>
      ...
      {showResetModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gold/20 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="reset-crm-title">
          <div ref={resetRef} className="w-full max-w-md rounded-[24px] border border-danger/30 bg-surface2 p-6 shadow-xl backdrop-blur-md space-y-5">
            <div className="flex items-center justify-between">
              <h2 id="reset-crm-title" className="text-xl font-display font-bold uppercase tracking-tight text-danger">Factory Reset CRM</h2>
              <button onClick={() => setShowResetModal(false)} aria-label="Close reset confirmation" className="w-9 h-9 rounded-full hover:bg-surface2 flex items-center justify-center">
                <X size={16} />
              </button>
            </div>
            <p className="text-sm text-text2">Type RESET to confirm. This screen is intentionally blocked because no backend reset endpoint is connected.</p>
            ...
            <button
              ...
              className="w-full rounded-xl border border-danger/30 bg-dangerSoft py-4 text-[10px] font-mono font-bold uppercase tracking-widest text-danger cursor-not-allowed"
            >
              {resetText === 'RESET' ? 'Backend reset endpoint not connected.' : 'Type RESET to continue'}
            </button>
            <p className="text-[10px] font-mono font-bold uppercase tracking-widest text-danger">Backend reset endpoint not connected.</p>
          </div>
        </div>
      )}
    </CrmShell>
